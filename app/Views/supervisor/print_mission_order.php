...
<body onload="window.print()">
    ...
    <div class="header">
        <div class="preview-header-dynamic">
            ...
            <div class="preview-agency-info">
                <div style="font-weight: bold; font-size: 14px; margin-bottom: 5px;"><?= $data['order']->agency_name ?? $data['tenant']->name ?></div>
                <div style="white-space: pre-wrap;"><?= $data['order']->agency_address ?? $data['tenant']->address ?></div>
                <div style="margin-top: 5px;">Tél: <?= $data['order']->agency_phone ?? $data['tenant']->phone ?></div>
            </div>
        </div>
    </div>

    <div class="doc-title">
        <h1>Ordre de Mission N°<?= $data['order']->order_number ?></h1>
    </div>
    ...
    <div class="content-section">
        <p><span class="label">NOMS :</span> 
            ...
            <?php else: ?>
                <strong><?= $data['order']->prenom . ' ' . $data['order']->nom ?></strong>
            <?php endif; ?>
            </span>
        </p>
        <p><span class="label">FONCTION :</span> 
            <span class="value"><?= $data['order']->signatory_role ?? ($data['employee'] ? ($data['employee']->poste_name ?? 'Agent') : 'Equipe de mission') ?></span>
        </p>
        <p><span class="label">OBJET :</span> <span class="value"><?= $data['order']->object ?></span></p>
        <p><span class="label">LIEUX :</span> <span class="value"><?= $data['order']->itinerary ?? '-' ?></span></p>
        ...
        <p><span class="label">MOYEN DE DÉPLACEMENT :</span> <span class="value"><?= $data['order']->means_of_transport ?? '-' ?></span></p>
    </div>

    <div class="content-section" style="text-align: justify; margin-top: 60px; font-size: 16px; line-height: 1.6;">
        <p><?= $data['order']->footer_text ?? 'Nous prions aux Autorités Politico-Administratives, militaires et policières de faciliter libre passage, d’apporter assistance et accorder l’immunité liée aux fonctions du porteur de ce document.' ?></p>
    </div>

    <div class="footer-signatures">
        <div class="date-line">
            Fait à <?= $data['order']->sign_city ?? 'Kinshasa' ?>, le <?= $data['order']->validated_at ? (new IntlDateFormatter('fr_FR', IntlDateFormatter::LONG, IntlDateFormatter::NONE))->format(new DateTime($data['order']->validated_at)) : (new IntlDateFormatter('fr_FR', IntlDateFormatter::LONG, IntlDateFormatter::NONE))->format(new DateTime()) ?>
        </div>
        <div class="signature-block">
            <p><strong>Pour <?= $data['order']->agency_name ?? $data['tenant']->name ?>,</strong></p>
            ...
            <p style="margin-top: 10px;"><strong><?= $data['order']->signatory_name ?? 'NGUBI Mac' ?></strong></p>
            <p><strong><?= $data['order']->signatory_role ?? 'Managing Director' ?></strong></p>
        </div>
    </div>
